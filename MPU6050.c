...
/**
  * @brief  Write 8 bit to the sensor and intented to be called
						only within library
  * @param  DataWrite: 8 bit data to be written to the device
  * @param  Address: Target device address
  * @retval No return
  */
void Write(uint8_t DataWrite,uint8_t Address)
{
	uint8_t buff[2];
	buff[0]=Address;
	buff[1]=DataWrite;
	uint16_t DevAddress=MPUADDRESS<<1;
	HAL_I2C_Master_Transmit(&i2chandle,DevAddress,buff,5,200000);
}
...
void Read(uint8_t *Dataptr,uint8_t Address,uint16_t NoOfBytes)
{
	...
	buff[0]=Address;
	uint16_t DevAddress=MPUADDRESS<<1;
	ret=HAL_I2C_Master_Transmit(&i2chandle,DevAddress,buff,1,10);
...
void MPU6050_Config(MPU_Config_Struct *Config)
{
	...
	HAL_Delay(100);
	uint8_t buf=Config->CLKSRC&0x03;
	...
	buf=0x00;
	buf=Config->ACCEL_FS<<3;
	buf&=0x18;
	Write(buf,ACCEL_CONFIG);
	
	if(Config->SMPRT==0)
		Config->SMPRT=0x04;
	Write_SampleRate(Config->SMPRT);
